...
@property (nonatomic, strong) UILabel *startTimeLabel;//播放进度时间
@property (nonatomic, strong) UILabel *totalTimeLabel;//播放总时间时间
...
- (void)becomeActiveAction:(NSNotification *)sender{
     ...
     [self.bigPlayButton setImage:img forState:UIControlStateNormal];
...
- (void)prepareToPlayAsset:(AVURLAsset *)asset{
    ...
    self.bigPlayButton.selected = NO;
    if ([self respondsToSelector:@selector(bigPlayerAction:)]) {
        [self bigPlayerAction:self.bigPlayButton];
    }
...
//     [_activityIndicatorView startAnimating];
    [self.playerLoadingView startRotatingAndDefaultBg];
...
- (void)addViewTapAction{
    ...
    self.moreView.hidden = YES;
     ...
     //播放事件
    [self.bigPlayButton addTarget:self action:@selector(bigPlayerAction:) forControlEvents:UIControlEventTouchUpInside];
 ...
 */
- (void)bigPlayerAction:(UIButton *)sender{
...
- (void)sliderDown:(UISlider *)sender{
     [self.player pause];
    [self.bigPlayButton setImage:[UIImage imageNamed:CH_playIcon] forState:UIControlStateNormal];
    self.bigPlayButton.selected = NO;
     [self repeatsTimer];
}
...
- (void)sliderAction:(UISlider *)sender{
    ...
    [self.bigPlayButton setImage:[UIImage imageNamed:CH_pauseIcon] forState:UIControlStateNormal];
    self.bigPlayButton.selected = YES;
...
-(void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
     ...
     if ([keyPath isEqualToString:@"status"]) {//观察播放状态
         ...
         [self.playerLoadingView endRotating];
         self.bigPlayButton.hidden = NO;
          ...
          if ([playerItem status] == AVPlayerStatusReadyToPlay) {
               ...
               NSString *cuTime = [NSString stringWithFormat:@"%@",_totalTime];
               self.totalTimeLabel.text = cuTime;
...
- (CGFloat)returnTimeWidth{
     ...
     CGFloat timeWidth = [self.totalTimeLabel.text widthWithStringAttribute:fontInfo];
     return timeWidth+2;
}
...
- (void)monitoringPlayback:(AVPlayerItem *)playerItem {
     ...
     if (!self.playbackTimeObserver) {
          ...
          self.playbackTimeObserver  = [self.player addPeriodicTimeObserverForInterval:CMTimeMake(1, 1) queue:NULL usingBlock:^(CMTime time) {
               ...
               NSString *cuTime = [NSString stringWithFormat:@"%02d:%02d",(int)current / 60,(int)current % 60];
               weakSelf.startTimeLabel.text = cuTime;
...
- (void)moviePlayDidEnd:(NSNotification *)sender{
     ...
     [self.player seekToTime:CMTimeMake(0, 1) completionHandler:^(BOOL finished) {
         ...
         [weakSelf.bigPlayButton setImage:[UIImage imageNamed:CH_playIcon] forState:UIControlStateNormal];
...
- (void)initialization {
    ...
    //加载视图
    self.playerLoadingView = [[PlayerLoadingView alloc] init];
    [self addSubview:self.playerLoadingView];
    WEAKSELF
    self.playerLoadingView.tapLoadingViewBlock = ^{
        [weakSelf tapLoadingViewClick];
    };
    [self.playerLoadingView mas_makeConstraints:^(MASConstraintMaker *make) {
     ...
     if (self.playerType != PlayerTypeOfFullScreen) {//全屏类型 无锁屏控件
          ...
          [self.lockButton mas_makeConstraints:^(MASConstraintMaker *make) {
               make.right.equalTo(self.container.mas_right).offset(-kSpace20);
...
//               make.right.equalTo(self.container.mas_right).offset(-10);
//               make.width.height.equalTo(self.backView.mas_width);
//          }];
...
//          moreIcon.image = [UIImage imageNamed:CH_nofull_moreIcon];
          ...
          moreLabel.text = @"...";
          moreLabel.textColor = kWhite;
          moreLabel.textAlignment = NSTextAlignmentCenter;
          [self.moreView addSubview:moreLabel];
    ...
    [self.bigPlayButton setImage:[UIImage imageNamed:CH_playIcon] forState:UIControlStateNormal];
    self.bigPlayButton.backgroundColor = [UIColor colorWithDisplayP3Red:0 green:0 blue:0 alpha:0.3];
    self.bigPlayButton.layer.masksToBounds = YES;
    self.bigPlayButton.layer.cornerRadius  = 25;
    [self.container addSubview:self.bigPlayButton];
...
//        make.width.height.equalTo(@(40));
     ...
     [self.progressView mas_makeConstraints:^(MASConstraintMaker *make) {
          ...
          make.left.equalTo(self.startTimeLabel.mas_right).offset(10);
          make.centerY.equalTo(self.bottomView.mas_centerY);
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event{
     ...
     if (_gestureType == GestureTypeOfNone) {
          ...
          }else if ((currentLocation.x < frame.size.height*0.8) && (ABS(offset_x) <= ABS(offset_y))){            _gestureType = GestureTypeOfBrightness;
     ...
     else if ((_gestureType == GestureTypeOfBrightness) && (currentLocation.x < frame.size.height*0.8) && (ABS(offset_x) <= ABS(offset_y))){//调节亮度
...
- (void)showFastView{
     self.fastLabel.text = self.startTimeLabel.text;
     [UIView animateWithDuration:CH_annimationTime animations:^{
